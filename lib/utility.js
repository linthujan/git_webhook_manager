...
function generateAccessToken(user, isRemember = undefined, days = undefined) {
  if (!days) {
    ...
    return jwt.sign(user, authPrivateKey, { expiresIn: days });
  }
}

const generateToken = (id) => generateAccessToken({ id: bcrypt.hashSync(id, 10) });
    ...
    (err, user) => {
      if (err) {
        ...
        if (err.name == 'TokenExpiredError') {
          return next(new AppError("Authentication Expired! Please authenticate again!", 401));
        }
...
/**
 * 
 * @param {number} bytes in byte
 * @param {number} decimals decimal points, default "2"
 * @returns would be like "5 GB"
 */
...
/**
 * optional format string ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB']
 * @param {string} size should be like "5 GB"
 * @returns byte
 */
function parseBytes(size) {
  ...
  if (parts.length !== 2 || !sizes.includes(parts[1])) {
    throw new Error('Invalid byte format');
  }

  const sizeIndex = sizes.indexOf(parts[1]);
  const bytes = parseFloat(parts[0]) * Math.pow(1024, sizeIndex);

  return bytes;
}

function convertBytes(size, targetUnit) {
  try {
    const sizes = ['Bytes', 'KB', 'MB', 'GB', 'TB', 'PB', 'EB', 'ZB', 'YB'];
    const parts = size.split(' ');
    ...
    const sourceUnitIndex = sizes.indexOf(parts[1]);
    const bytes = parseFloat(parts[0]) * Math.pow(1024, sourceUnitIndex);

    if (!sizes.includes(targetUnit)) {
      throw new Error('Invalid target unit');
    }

    const targetUnitIndex = sizes.indexOf(targetUnit);
    const convertedSize = bytes / Math.pow(1024, targetUnitIndex);
...
// @ts-ignore
function response(req, res, error, data, meta, statusCode) {
  if (error) {
    if (!statusCode)
      res.statusCode = RESPONSE_CODE.INTERNAL_SERVER_ERROR;
    else
    ...
    //   res.statusCode = RESPONSE_CODE.NO_CONTENT;
    // } else {
...
function generatePassword(passwordLength) {
  ...
  } while (!regex.test(password) && password.length !== passwordLength);

  return password;
}

function getHoursDifferent(startDate, endDate) {
...
function getDayDifferent(startDate, endDate) {
  return getHoursDifferent(startDate, endDate) / 24;
}
...
function validateParams(req, next) {
  if (!req.params.id || !req.body) {
    return next(new AppError(AppError.NO_PARAMS, 400));
  }
  return true;
}

function validateValue(values, allowNull = false) {
  Object.keys(values).forEach((key) => {
    if (allowNull && values[key] === null) return;
...
function nullValueCheck(values, allowNull = false, emptyString = false) {
  let output = [];
  Object.keys(values).forEach((key) => {
    if (emptyString && values[key] === "") return;
    if (allowNull && values[key] === null) return;
    if (values[key] === undefined || !values[key] || values[key] === "") {
      delete values[key];
    } else {
...
function ConvertStringToImage(base64) {
  var base64Data = base64.replace(/^data:image\/png;base64,/, "");

  let buff = Buffer.from(base64Data, 'base64');
  // require('fs').writeFileSync(`data/${products[i].br_code}.png`, buff);
...
const SMS_TEMPLATE = {
  OTP: "Your OTP for registration is {0}. This OTP is valid for {1} minutes"
}
